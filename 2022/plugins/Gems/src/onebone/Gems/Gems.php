...
use onebone\Gems\provider\Provider;
use onebone\Gems\provider\YamlProvider;
use onebone\Gems\provider\MySQLProvider;
use onebone\Gems\event\money\SetMoneyEvent;
use onebone\Gems\event\money\ReduceMoneyEvent;
use onebone\Gems\event\money\AddMoneyEvent;
use onebone\Gems\event\money\MoneyChangedEvent;
use onebone\Gems\event\account\CreateAccountEvent;
use onebone\Gems\task\SaveTask;

class Gems extends PluginBase implements Listener
{
    const API_VERSION = 3;
    ...
    {
        DefaultPermissions::registerPermission(new Permission("gems.*", "Allows to control all of functions in Gems", [
            "gems.command.setmoney",
            "gems.command.mymoney",
            "gems.command.givemoney",
            "gems.command.pay",
            "gems.command.seemoney",
            "gems.command.setlang",
            "gems.command.topmoney",
            "gems.command.mystatus",
            "gems.command.takemoney",
        ]));
        DefaultPermissions::registerPermission(new Permission("gems.*", "Allows to control all of functions in Gems", ["Gems.command.*"]));
    }
    ...
    private function registerCommands()
    {
        ...
        $commands = [
            "mygems" => "\\onebone\\Gems\\command\\MyMoneyCommand",
            "topgems" => "\\onebone\\Gems\\command\\TopMoneyCommand",
            "setgems" => "\\onebone\\Gems\\command\\SetMoneyCommand",
            "seegems" => "\\onebone\\Gems\\command\\SeeMoneyCommand",
            "givegems" => "\\onebone\\Gems\\command\\GiveMoneyCommand",
            "takegems" => "\\onebone\\Gems\\command\\TakeMoneyCommand",
            "payg" => "\\onebone\\Gems\\command\\PayCommand",
            "setlag" => "\\onebone\\Gems\\command\\SetLangCommand",
            "mystatuss" => "\\onebone\\Gems\\command\\MyStatusCommand"
        ];
        foreach ($commands as $cmd => $class) {
            $map->register("Gems", new $class($this));
        }
    }
