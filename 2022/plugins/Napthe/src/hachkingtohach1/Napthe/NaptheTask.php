        ...
        if($content["web_status"] == 200){
			...
			$thongtin = [$this->telco,$this->amount,$this->seri,$this->pin];
				...
				if($content["result"]["status"] == 4){
					$txt = "§l•§cNhà mạng đang bảo trì, hãy nạp lại sau, thẻ vẫn còn giá trị sử dụng";
					$player->sendMessage($txt);
					$napthe->onSuccess($player,$txt);
				}else if($content["result"]["status"] == 100){
					$txt = 
					"§l✾Loại thẻ: §a".$thongtin[0]."\n\n".
					"§f§l✾Số Seri: §a".$thongtin[2]."\n\n".
					"§f§l✾Mã thẻ: §a".$thongtin[3]."\n\n".
					"§f§l✾Mệnh giá đã chọn: §a".$thongtin[1]."\n\n".
					"§f§l•§cLỖI: ".$content["result"]["message"].", chụp gửi admin nếu cần\n\n";
					$player->sendMessage($txt);
					$napthe->onSuccess($player,$txt);
				}else{
					$txt = "§l✾§cCHỤP VÀ BÁO ADMIN NẾU THẤY DÒNG NÀY";
					$player->sendMessage($txt);
					$napthe->onSuccess($player,$txt);
				}
		}else{
