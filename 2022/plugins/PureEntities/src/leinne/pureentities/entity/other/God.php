...
class God extends Monster{
    ...
    protected function initEntity(CompoundTag $nbt) : void{
        ...
        $this->setSpeed(2);
        $this->setDamages([10, 20, 30, 50]);
    }

    public function getName() : string{
        return TextFormat::GOLD."God";
    }
				...
				foreach($this->getWorld()->getEntities() as $entity){	
                    ...
                    if($entity->getPosition()->distance($vector) <= 3){
					    if($entity instanceof Player){
		                    ...
		                    $entity->getWorld()->addParticle($vectorNew, new HugeExplodeParticle(), [$entity]);
					        $ev = new EntityDamageByEntityEvent($this, $target, EntityDamageEvent::CAUSE_ENTITY_ATTACK, $this->getResultDamage() * 2);
                            $target->attack($ev);
						}                       						
				    }
			    }
