...
class Quest extends PluginBase implements Listener{
	...
	public function getQuestPlayer(Player $player) :string{
		...
		if(!empty($this->getQuest($player))){
		    $quest = $this->getConfig()->get("ALL_QUESTS_DONE")[0];
			if($this->getQuest($player) <= $this->getConfig()->get("MAXQUEST")){
                if(!empty($this->getConfig()->get("QUESTS")[$this->getQuest($player)])){
	...
	public function getProcessQuestPlayer(Player $player){		
	    $this->getDataBase()->createProfile($player);
		$process = 0;
		if(!empty($this->getQuest($player))){
			if(!isset($this->getConfig()->get("QUESTS")[$this->getQuest($player)])){
			    return $process = $this->getConfig()->get("ALL_QUESTS_DONE")[1];
			}
	...
	public function rewardQuest(Player $player, array $cmds){
		foreach($cmds as $cmd){
		    Server::getInstance()->dispatchCommand(new ConsoleCommandSender($this->getServer(), $this->getServer()->getLanguage()), str_replace("%player", $player->getName(), $cmd));
		}
	}
	...
	public function checkQuest(Player $player, $event) :bool{
		...
		if($quest["EVENT"] == "break" and $event instanceof BlockBreakEvent){			
			if($event->getBlock()->getId() == (int)$quest["ID"] and $event->getBlock()->getMeta() == (int)$quest["META"]){
