...
class InviteSubCommand extends BaseSubCommand
{
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
		...
		$skyblockPlayer = SkyBlocksPM::getInstance()->getPlayerManager()->getPlayer($sender)->getSkyblock();
		$skyblock = SkyBlocksPM::getInstance()->getSkyBlockManager()->getSkyBlockByWorld(SkyBlocksPM::getInstance()->getServer()->getWorldManager()->getWorldByName($skyblockPlayer));
        if ($skyblockPlayer == '')
        {
        ...
        {
            $sender->sendMessage(SkyBlocksPM::getInstance()->getMessages()->getMessage('player-not-online'));
            return;
        }
        if ($sender === $player)
            return;
