...
class PartyOptionsForm extends CustomForm {
    ...
    protected function onCreation(): void {
        ...
        $this->addElement("label", new Label("Thay đổi chỉnh sửa Party."));
        $this->addElement("public", new Toggle("Bạn muốn để Party dưới dạng public?", $party->isPublic()));
        if(ConfigGetter::isPvpDisabledOption()) {
            $this->addElement("disable_pvp", new Toggle("Bạn muốn tắt gây damage lến tất cả thành viên trong party?", !$party->isPvp()));
        }
        if(ConfigGetter::isWorldTeleportOption()) {
            $this->addElement("leader_world_teleport", new Toggle("Bạn muốn triệu tập tất cả thành viên trong party đến bạn?", $party->isLeaderWorldTeleport()));
        }
        $this->addElement("slots", new Slider("Thiết lập giới hạn số lượng thành viên", 1, ConfigGetter::getMaximumSlots(), $party->getSlots(), 1));
    }
