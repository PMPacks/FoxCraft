  ...
  public function onTouch(PlayerInteractEvent $event) {
    ...
    if($this->isBanned($event->getItem())) {
      if(!($p->hasPermission("banitem") || $p->hasPermission("banitem.bypass"))) {
        $p->sendMessage("§bVật Phẩm Đã Bị Cấm");
        $event->setCancelled();
      }
    }
  }
  ...
  public function onCommand(CommandSender $p,Command $cmd,$label,array $args) {
    ...
    if(strtolower($args[0]) == "ban" || strtolower($args[0]) == "unban") {
      ...
      if(!is_numeric($item[0]) || (isset($item[1]) && !is_numeric($item[1]))) {
        $p->sendMessage("§bVui Lòng Chỉ Sử Dụng ID Vật Phẩm Hoặc Tên Vật Phẩm Không Sử Dụng Các Kí Tự Đặt Biệt Trừ Kí Tự _");
        return true;
      }
    }
    if(strtolower($args[0]) == "ban") {
      ...
      if(in_array($i,$this->items)) {
        ...
        $this->saveItems();
        $p->sendMessage("§bVật Phẩm Mang ID:§c " . str_replace("#",":",$i) . " §bĐã Bị Cấm");
      }
      ...
      $p->sendMessage("§bDanh Sách Vật Phẩm Bị Ban" . (count($this->items) == 1 ? "" : "") . ": §a" . str_replace("#", ":", implode(", ", $this->items)) . (count($this->items) > 0 ? "." : "§cTrống."));
    } else {
      return false;
    }
    return true;
